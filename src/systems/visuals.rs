    ...
    for &pos in shape {
        ...
        commands.entity(entity).with_children(|parent| {
            ...
            let directions = [
                (
                    IVec2::X,
                    Vec2::new(bridge_ext, bridge_width),
                    Vec2::new(inner_size / 2.0 + bridge_ext / 2.0, 0.0),
                ),
                (
                    IVec2::NEG_X,
                    Vec2::new(bridge_ext, bridge_width),
                ...
                (
                    IVec2::Y,
                    Vec2::new(bridge_width, bridge_ext),
                    Vec2::new(0.0, inner_size / 2.0 + bridge_ext / 2.0),
                ),
                (
                    IVec2::NEG_Y,
                    Vec2::new(bridge_width, bridge_ext),
            ...
            for (dir, b_size, b_offset) in directions {
                if shape.contains(&(pos + dir)) {
                    ...
                    let line_size = if is_horizontal {
                        Vec2::new(TILE_SIZE, line_thickness)
                    } else {
                        Vec2::new(line_thickness, TILE_SIZE)
                    };
                    let line_offset = dir.as_vec2() * (TILE_SIZE / 2.0);

                    parent.spawn((
                        Sprite {
                            color: perimeter_color.into(),
